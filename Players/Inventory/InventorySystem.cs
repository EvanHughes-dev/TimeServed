...
namespace TimeServed.Players.Inventory
{
    internal class InventorySystem
    {
        ...
        public InventorySystem(Point screenSize)
        {
            _selectedSpace = null;
        ...
        public void Draw(SpriteBatch sb, Point screenSize)
        {
            for (int i = 0; i < _inventoryUI.Length; i++)
            {
                _inventoryUI[i].UpdateRectangle(new Rectangle(screenSize.X / 2 - (_boxSize * 2) + (_boxSize * i), screenSize.Y - (_boxSize + 30), _boxSize, _boxSize));
